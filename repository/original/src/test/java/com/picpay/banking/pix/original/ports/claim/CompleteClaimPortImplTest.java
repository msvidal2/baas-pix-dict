...
class CompleteClaimPortImplTest {
    ...
    @Test
    void when_completeWithSuccess_expect_claim() {
        var responseWrapper = ResponseWrapperDTO.<ClaimResponseDTO>builder()
                .data(claimResponseDTO)
                .build();

        when(claimClient.finish(anyString(), any())).thenReturn(responseWrapper);

        assertDoesNotThrow(() -> {
            var response = port.complete(claim, randomUUID().toString());
    ...
    @Test
    void when_completeReturnWithoutClaim_expect_null() {
