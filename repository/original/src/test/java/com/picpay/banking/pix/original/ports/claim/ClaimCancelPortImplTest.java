...
class ClaimCancelPortImplTest {
    ...
    @Test
    void when_cancelWithSuccess_expect_claim() {
        var responseWrapper = ResponseWrapperDTO.<ClaimResponseDTO>builder()
                .data(claimResponseDTO)
                .build();

        when(claimClient.cancel(anyString(), any())).thenReturn(responseWrapper);

        assertDoesNotThrow(() -> {
            var response = port.cancel(claim, false, CLIENT_REQUEST, randomUUID().toString());
    ...
    @Test
    void when_cancelReturnWithoutClaim_expect_null() {
        var responseWrapper = ResponseWrapperDTO.<ClaimResponseDTO>builder()
                .build();
