...
import static com.picpay.banking.pix.core.events.EventType.PIX_KEY_CREATED_BY_SYNC;
import static com.picpay.banking.pix.core.events.EventType.PIX_KEY_REMOVED_BY_SYNC;
import static com.picpay.banking.pix.core.events.EventType.PIX_KEY_UPDATED_BY_SYNC;
...
public class ReconciliationActionPortImpl implements ReconciliationActionPort {
    ...
    public void insertPixKey(final SyncVerifierHistoricAction pixKeyAction) {
        ...
        pixKeyEventRegistryUseCase.execute(PIX_KEY_CREATED_BY_SYNC, UUID.randomUUID().toString(),
            PixKeyEventData.from(pixKeyInserted, RECONCILIATION));
    }
    ...
    public void updatePixKey(final SyncVerifierHistoricAction oldPixKeyAction, final SyncVerifierHistoricAction newPixKeyAction) {
        ...
        pixKeyEventRegistryUseCase.execute(PIX_KEY_UPDATED_BY_SYNC, UUID.randomUUID().toString(),
            PixKeyEventData.from(pixKeyUpdated, RECONCILIATION));
    }
    ...
    public void removePixKey(final SyncVerifierHistoricAction pixKeyAction) {
        pixKeyRepository.findByCidAndReasonNot(pixKeyAction.getCid(), Reason.INACTIVITY)
            .ifPresent(pixKeyEntity -> {
                ...
                syncVerifierHistoricActionRepository.save(SyncVerifierHistoricActionEntity.from(pixKeyAction));
                pixKeyEventRegistryUseCase.execute(PIX_KEY_REMOVED_BY_SYNC, UUID.randomUUID().toString(),
