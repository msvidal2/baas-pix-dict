...
class CreatePixKeyUseCaseTest {
    ...
    void when_executeWithSuccess_expect_pixKeyWithCreatedAt() {
        when(createPixKeyPort.createPixKey(any(), any(), anyString()))
                .thenReturn(pixKeyCreated);

        assertDoesNotThrow(() -> {
            var response = useCase.execute(pixKey, CUSTOMER_REQUEST, randomUUID().toString());
    ...
    void when_executeWithNullRequestIdentifier_expect_exception() {
        assertThrows(IllegalArgumentException.class, () -> useCase.execute(pixKey, CUSTOMER_REQUEST, null));
    }
