...
class FilterInfractionReportUseCaseTest {
    ...
    void setup() {
        ...
        infractionPage = InfractionPage.builder()
            .content(Collections.singletonList(infractionReport))
            .size(1)
            .offset(0)
            .pageSize(1)
            .build();
    ...
    void when_filterInfractionsWithSuccess_expect_OkWithValidResult() {
        when(infractionReportPort.list(any(), any(), any(),anyInt(),anyInt())).thenReturn(infractionPage);

        var infractionPage = this.filterInfractionReportUseCase.execute(InfractionReportSituation.ANALYZED, LocalDateTime.parse("2020-09-01T10:09:49.922138"), null,1,1);
        assertThat(infractionPage.getContent()).isNotEmpty();

        verify(infractionReportPort).list(any(), any(), any(),anyInt(),anyInt());
    }
