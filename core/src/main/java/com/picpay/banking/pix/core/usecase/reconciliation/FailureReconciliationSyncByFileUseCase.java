    ...
    public SyncVerifierHistoric execute(KeyType keyType) {
        try {
            ...
            if (!CollectionUtils.isEmpty(resultCidFile.getContent()) && resultCidFile.isNotProcessed()) {
                try {
                    return processFile(resultCidFile);
                } catch (Exception e) {
        ...
        return SyncVerifierHistoric.builder().syncVerifierResultType(SyncVerifierResultType.NOK).build();
    }

    private SyncVerifierHistoric processFile(final ContentIdentifierFile contentIdentifierFile) {
